...
- **Enchantment glint on bookshelf slots redrawn for 26.3.** 26.3 removed the standalone glint layer the overlay used, folding glint into the material itself, so the glint is now drawn in a single pass instead of two. The effect should look the same — worth a glance if you run a resource pack that restyles chiseled bookshelves.
